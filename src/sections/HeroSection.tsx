import { Heart, Sparkles, Wand2, LayoutTemplate, Bell, ArrowRight, MousePointerClick, Palette, Send } from 'lucide-react'
...
export function HeroSection() {
  ...
  const steps = [
    { icon: MousePointerClick, text: 'Chọn mẫu', sub: '500+ mẫu thiệp' },
    { icon: Palette, text: 'Tùy chỉnh', sub: 'Thêm ảnh & lời' },
    { icon: Send, text: 'Gửi ngay', sub: 'Link / Email / Zalo' },
  ]
  ...
  return (
    <section className="relative z-10 mx-auto grid min-h-[680px] max-w-[1440px] overflow-hidden sm:min-h-[720px] lg:min-h-[780px] lg:grid-cols-[1fr_1fr]">
      {/* Mobile-only marquee background. The layered cream gradient keeps
          the foreground copy readable while the imagery remains visible. */}
      <div aria-hidden="true" className="absolute inset-0 -z-10 overflow-hidden bg-[#fdf2e3] lg:hidden">
        <div className="absolute inset-0 grid scale-[1.01] grid-cols-2 gap-2 opacity-[0.68] blur-0">
        ...
        <div className="absolute inset-0 bg-gradient-to-b from-[#fcfbf8]/82 via-[#fdf2e3]/28 to-[#fdf2e3]/08"/>
        <div className="absolute inset-x-0 bottom-0 h-2/5 bg-gradient-to-t from-[#f7c948]/15 to-transparent"/>
      </div>

      <div className="flex flex-col justify-center px-4 pt-8 pb-20 sm:px-8 lg:px-14">
        ...
        <ScrollReveal direction="left" delay={0.55} distance={28}>
          <div className="mt-6 sm:mt-8">
            ...
            {/* Guide Steps */}
            <div className="mt-5 flex items-center justify-center gap-4 sm:justify-start sm:gap-6">
              {steps.map(({ icon: Icon, text, sub }, i) => (
                <div className="flex flex-col items-center gap-1 sm:flex-row sm:gap-3" key={text}>
                  {i > 0 && (
                    <div className="hidden h-4 w-4 rotate-90 text-[#d9a441]/40 sm:block sm:rotate-0">
                      <ArrowRight size={16} />
                    </div>
                  )}
                  <div className="flex flex-col items-center sm:flex-row sm:gap-2">
                    <span className="grid size-8 shrink-0 place-items-center rounded-full bg-[#d9a441]/15 text-[#d9a441] sm:size-9">
                      <Icon size={16} className="sm:size-[18px]"/>
                    </span>
                    <div className="text-center sm:text-left">
                      <span className="text-xs font-semibold text-[#7c3f06] sm:text-sm">{text}</span>
                      <span className="ml-1 hidden text-xs text-[#7c3f06]/60 sm:inline">{sub}</span>
                    </div>
                  </div>
                </div>
              ))}
